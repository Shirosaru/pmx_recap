...
# --- Configuration ---
BASE_DIR = "/home2/mmGBSA/test6_different"
...
def prepare_system(source_pdb_path, workdir):
    ...
    run_cmd(f"{GROMACS} grompp -f {MDP_DIR}/minim.mdp -c solv_ions.gro -p topol.top -o em.tpr  -maxwarn 1", cwd=workdir)
...
def create_chain_index_by_residues(tpr_file_path, ndx_file_path, chainA_range, chainB_range, zinc_resids, cwd):
    ...
    return chain_a_group_num,  chain_b_group_num

# run_gmx_mmpbsa function (same as before)
def run_gmx_mmpbsa(mmpbsa_in, complex_tpr_file, xtc_file, topol_file, ndx_file, chain_a_group_num, chain_b_group_num):
    ...
    cmd = [
        ...
        '-cp', topol_file,
        '-ci', ndx_file,
        '-cg', str(chain_a_group_num), str(chain_b_group_num),
    ]
...
if __name__ == "__main__":
    ...
    try:
        ...
        # ✅ Capture returned group numbers
        ...
        # ✅ Pass group numbers to gmx_MMPBSA
        run_gmx_mmpbsa(
            mmpbsa_in=wt_mmpbsa_input,
            complex_tpr_file=wt_tpr_for_gmxmmpbsa_cs,
            xtc_file=wt_xtc_for_gmxmmpbsa_ct,
            topol_file=wt_protein_only_topol_for_mmpbsa,
            ndx_file=wt_ndx,
    ...
    try:
        ...
        # ✅ Pass them to run_gmx_mmpbsa
        run_gmx_mmpbsa(
            mmpbsa_in=mt_mmpbsa_input,
            complex_tpr_file=mt_tpr_for_gmxmmpbsa_cs,
            xtc_file=mt_xtc_for_gmxmmpbsa_ct,
            topol_file=mt_protein_only_topol_for_mmpbsa,
            ndx_file=mt_ndx,
            chain_a_group_num=chain_a_group_num,
            chain_b_group_num=chain_b_group_num
        )
