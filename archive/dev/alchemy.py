...
def _set_conformation(old_res, new_res, rotdic):
    ...
    for key in rotdic.keys():
        at1, at2 = key.split('-')
        for d in dihedrals:
            if d[1] == at1 and d[2] == at2 and d[-1] != -1:
                chis.append(d)
    ...
    print("\n🧪 DEBUG: Mapping atom coordinates from old_res to new_res")
    old_atom_names = [atom.name for atom in old_res.atoms]
    new_atom_names = [atom.name for atom in new_res.atoms]
    print(f"  → Old residue atoms: {old_atom_names}")
    print(f"  → New residue atoms: {new_atom_names}")
    ...
    for atom in new_res.atoms:
        if (atom.name[0] != 'D') and (not atom.name.startswith('HV')):
            try:
                ...
                print(f"⚠️  Atom '{atom.name}' not found in old_res '{old_res.resname}' (ID {old_res.id}) — skipping.")
            except Exception as e:
                print(f"❌ Unexpected error copying coordinates for atom '{atom.name}': {e}")
...
def _get_hybrid_residues(m, ff, version='new', verbose=False):
    ...
    for res in m.residues:
        if res.is_hybrid():
            ...
            if len(atoms) != len(hybrid_res.atoms):
                print(f"\n⚠️ Skipping hybrid residue {res.resname} (ID: {res.id}) due to atom mismatch:")
                print(f"  → Expected {len(hybrid_res.atoms)} atoms, got {len(atoms)}")
                missing = set(atom_names) - set(a.name for a in atoms)
                print(f"  → Missing atoms: {', '.join(sorted(missing))}")
                continue
